...
use std::ffi::{CString};
use std::fs::{File, OpenOptions, copy, create_dir_all, read_link, remove_file};
...
impl MpiDistSyncGossipCommWorker {
  pub fn new(gossip_cfg: GossipConfig, context: Rc<DeviceContext>) -> MpiDistSyncGossipCommWorker {
    ...
    let curr_dir = match current_dir() {
      Err(e) => panic!("failed to query current working directory: {:?}", e),
      Ok(path) => path,
    };
    println!("DEBUG: rank: {} current dir: {:?}", worker_rank, curr_dir);
    let mut scratch_dir = curr_dir.clone();
    scratch_dir.push("tmp");
    let mut self_service_path = scratch_dir.clone();
    self_service_path.push(&format!("{}.port", worker_rank));
    {
      let mut service_file = File::create(&self_service_path).unwrap();
      write!(&mut service_file, "{}", service_port.to_str().unwrap());
    }
    ...
    for server_r in 0 .. num_workers {
      for client_r in 0 .. num_workers {
        ...
        if worker_rank == client_r {
          ...
          let mut service_path = scratch_dir.clone();
          service_path.push(&format!("{}.port", server_r));
          let mut service_file = File::open(&service_path).unwrap();
          let mut server_port_buf = vec![];
          service_file.read_to_end(&mut server_port_buf).unwrap();
          let server_port = CString::new(server_port_buf).unwrap();
