  ...
  pub fn run(mut self) {
    ...
    'poll_loop: loop {
      ...
      'chk_probe_loop: for &r in poll_ranks.iter() {
        ...
        match MpiComm::world().nonblocking_probe(Some(r), Some(2)) {
      ...
      'probe_loop: for &r in poll_ranks.iter() {
        ...
        match MpiComm::world().nonblocking_probe(Some(r), Some(1)) {
          Err(e) => panic!("async gossip: passive thread: failed to do first recv: {:?}", e),
          Ok(None) => {}
          Ok(Some(_)) => {
            recv_rank = Some(r);
...

      {
        ...
        let recv_req = match MpiComm::world().nonblocking_recv(&mut target_buf_h[ .. self.msg_len], Some(recv_rank), Some(1)) {
          Ok(req) => req,
          Err(e) => panic!("async gossip: passive thread: failed to do nonblocking recv: {:?}", e),
        };
        ...
        for msg in 1 .. self.num_buf_msgs {
          let recv_req = match MpiComm::world().nonblocking_recv(&mut target_buf_h[msg * self.msg_len .. (msg+1) * self.msg_len], Some(recv_rank), Some(0)) {
            Ok(req) => req,
            Err(e) => panic!("async gossip: passive thread: failed to do nonblocking recv: {:?}", e),
          };
          self.recv_reqs.append(recv_req);
...
impl CommWorker for MpiDistAsyncPushGossipCommWorker {
  ...
  fn communicate(&mut self/*, ctx: &DeviceCtxRef*/) {
    ...
    if self_rank != send_rank {
      ...
      self.send_reqs.clear();
      let send_req = match MpiComm::world().nonblocking_sync_send(&self.origin_buf_h[ .. self.msg_len], send_rank, 1) {
        Ok(req) => req,
        Err(e) => panic!("async gossip: active thread: failed to do initial send: {:?}", e),
      };
      self.send_reqs.append(send_req);
      ...
      for msg in 1 .. self.num_buf_msgs {
        let send_req = match MpiComm::world().nonblocking_send(&self.origin_buf_h[msg * self.msg_len .. (msg+1) * self.msg_len], send_rank, 0) {
          Ok(req) => req,
