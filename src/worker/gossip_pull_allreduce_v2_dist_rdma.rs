...
pub struct MpiDistAsyncPullGossipAllreduceV2CommWorker {
...
impl MpiDistAsyncPullGossipAllreduceV2CommWorker {
  pub fn new(group_size: usize, msg_len: usize, req_buf_len: usize, context: Rc<DeviceContext>) -> MpiDistAsyncPullGossipAllreduceV2CommWorker {
    ...
    MpiDistAsyncPullGossipAllreduceV2CommWorker{
...
impl MpiDistCommWorker for MpiDistAsyncPullGossipAllreduceV2CommWorker {
  fn mpi(&self) -> &Mpi {
    &self.mpi
  }
}

impl CommWorker for MpiDistAsyncPullGossipAllreduceV2CommWorker {
  ...
  fn communicate(&mut self) {
    ...
    for msg in 0 .. self.num_buf_msgs {
      let req = match self.group_comms[group_rank].nonblocking_allreduce_(
    ...
    if self_rank != recv_rank {
      ...
      self.dst_buf.as_ref_mut(ctx).row_vector_sum(1.0 / (self.group_size as f32), &self.src_buf.as_ref(ctx));
      self.dst_buf.as_ref_mut(ctx).row_vector_scale(0.5);
    } else {
      self.src_buf.as_ref(ctx).send(&mut self.dst_buf.as_ref_mut(ctx));
      self.dst_buf.as_ref_mut(ctx).row_vector_scale(1.0 / (self.group_size as f32));
    }
