...
impl MpiDistAsyncPullGossipCommWorker {
  pub fn new(gossip_cfg: GossipConfig, context: Rc<DeviceContext>) -> MpiDistAsyncPullGossipCommWorker {
    ...
    /*let passive_thr = {
      ...
      spawn(move || {
        let passive_worker = MpiDistAsyncPullGossipPassiveWorker{
          worker_rank:  worker_rank,
          num_workers:  num_workers,
          msg_len:      msg_len,
          num_buf_msgs: num_buf_msgs,
          rdma_buf_len: rdma_buf_len,
          state:        AsyncPullGossipPassiveState::Receiving,
...
impl CommWorker for MpiDistAsyncPullGossipCommWorker {
  ...
  fn communicate(&mut self/*, ctx: &DeviceCtxRef*/) {
    ...
    /*match self.act2pass_tx.send(AsyncPullGossipAct2PassMsg::Pause) {
    ...
    match self.pass2act_rx.recv() {
      ...
      Ok(AsyncPullGossipPass2ActMsg::AckPause) => {
        // Do nothing.
      }
    }*/
    ...
    if self_rank == recv_rank {
      ...
      //self.dst_buf.as_ref_mut(ctx).row_vector_add(1.0, &self.pull_origin_
    ...
    /*match self.act2pass_tx.send(AsyncPullGossipAct2PassMsg::Resume) {
      Ok(_) => {}
