...
fn main() {
  ...
  let mut ip1_layer = FullyConnLayer::new(Some(&data_layer), width * height, 10, ActivationFunction::Identity);
  let mut softmax_layer = SoftmaxLossLayer::new(Some(&ip1_layer), 10);*/

  let ip1_layer_cfg = FullyConnLayerConfig{in_channels: width * height, out_channels: 800, act_fun: ActivationFunction::Relu};
  let ip2_layer_cfg = FullyConnLayerConfig{in_channels: 800, out_channels: 10, act_fun: ActivationFunction::Identity};

  let mut data_layer = DataLayer::new(width * height);
  let mut ip1_layer = FullyConnLayer::new(Some(&data_layer), ip1_layer_cfg);
  let mut ip2_layer = FullyConnLayer::new(Some(&ip1_layer), ip2_layer_cfg);
