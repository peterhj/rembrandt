...
fn main() {
  ...
  let mut conv1_layer = Conv2dLayer::new(Some(&data_layer), conv1_layer_cfg, &ctx);
  let mut fc1_layer = FullyConnLayer::new(Some(&conv1_layer), fc1_layer_cfg);
  let mut fc2_layer = FullyConnLayer::new(Some(&fc1_layer), fc2_layer_cfg);
  let mut softmax_layer = SoftmaxLossLayer::new(Some(&fc2_layer), 10);

  conv1_layer.initialize_params(&ctx);
  fc1_layer.initialize_params(&ctx);
  fc2_layer.initialize_params(&ctx);
  ...
  loop {
    ...
    conv1_layer.reset_gradients(&ctx);
    fc1_layer.reset_gradients(&ctx);
    fc2_layer.reset_gradients(&ctx);
    ...
    for (idx, &(ref datum, maybe_label)) in train_data.iter().enumerate() {
      ...
      conv1_layer.forward(&ctx);
      fc1_layer.forward(&ctx);
      fc2_layer.forward(&ctx);
      ...
      fc2_layer.backward(&descent, &ctx);
      fc1_layer.backward(&descent, &ctx);
      conv1_layer.backward(&descent, &ctx);
      ...
      if next_idx >= num_epoch_samples {
        ...
        conv1_layer.descend(&descent, &ctx);
        conv1_layer.reset_gradients(&ctx);
        fc1_layer.descend(&descent, &ctx);
        fc1_layer.reset_gradients(&ctx);
        fc2_layer.descend(&descent, &ctx);
        fc2_layer.reset_gradients(&ctx);
      }
    }
